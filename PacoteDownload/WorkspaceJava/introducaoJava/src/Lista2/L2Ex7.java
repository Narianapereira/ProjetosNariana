...
public class L2Ex7 {
	...
	public static void main(String[] args) {
		...
		// atribui as matérias a um vetor para depois imprimir
		String[] materias = { "Matemática", "Física", "Química" };
		double maiorNota = 0;
		String disciplinaMaiorNota = "matemática";
		...
		for (l = 0; l < 4; l++) {
			for (c = 0; c < 3; c++) {
				// encontra a maior nota e armazena ela na variável maiorNota
				if (notas[l][c] > maiorNota) {
					maiorNota = notas[l][c];
					// armazena a disciplina da maior nota conforme a coluna em que está a maior
					// nota
					if (c == 1) {
...
// executa conforme opcao escolhida pelo usuário
			switch (opcoesMenu) {
			case 1:
			...
			case 2:
				...
				break;
			case 3:
				// escolhe a matéria que quer ver a média
				disciplinaMedia = Integer.parseInt(JOptionPane.showInputDialog(
						"Qual matéria você deseja ver a média? \n 1 - Matemática \n 2 - Física \n 3 - Química"));
				// soma das linhas da coluna escolhida através da variável disciplinaMedia
				for (l = 0; l < 4; l++) {
					somaMedia = somaMedia + notas[l][(disciplinaMedia - 1)];
				}
// calculo da media com a soma das linhas							
				media = somaMedia / 4;
				JOptionPane.showMessageDialog(null, "A média desta disciplina foi de " + df.format(media));

				break;
			case 4:
				// imprime a parte de cima da matriz
				opcaoBimestre = Integer
						.parseInt(JOptionPane.showInputDialog("Qual bimestre você deseja ver as notas?"));
				if (opcaoBimestre == 1) {
					// atribui os nomes das materias a string - concatena
					for (i = 0; i < 3; i++) {
						imprimeMaterias = imprimeMaterias + materias[i];
